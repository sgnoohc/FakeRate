...
//            return;
        if (leptons[0].isTTbar && leptons[0].motherID <= 0)
            return;
...
//        std::cout << leptons[0].isData << " " << leptons[0].isDoubleMuon << std::endl;
        ...
        if (leptons[0].evt_met < 20 && leptons[0].evt_mt < 20)
        {
            ...
            else if (abs(leptons[0].id) == 13)
            {
//                if (leptons[0].p4.pt() < 20)
//                {
//                    std::cout << leptons[0].pass_trig << " " << leptons[0].p4.pt() << std::endl;
//                    std::cout << leptons.size() << std::endl;
//                    std::cout << leptons[0].evt_event << " " << leptons[0].evt_lumiBlock << " " << leptons[0].evt_run << std::endl;
//                }
        ...
        // high MT window region
        if (evt_mt > 80. && evt_mt < 120. && leptons[0].p4.pt() > 25.)
        {
            if (abs(leptons[0].id) == 13)
            {
                hists.fill(leptons[0].nvtx, prefix + "evt_lvl_histo_nvtx_mtwindow_mu", leptons[0].weight, 50, 0, 50);
                hists.fill(evt_met, prefix + "evt_lvl_histo_met_mtwindow_mu", leptons[0].weight, 50, 0, 200);
                hists.fill(leptons[0].p4.pt(), prefix + "evt_lvl_histo_pt_mtwindow_mu", leptons[0].weight, 80, 0, 200);
                hists.fill((1 + leptons[0].coneptcorr) * leptons[0].p4.pt(), prefix + "evt_lvl_histo_conecorrpt_mtwindow_mu", leptons[0].weight, 80, 0, 200);
//                hists.fill(leptons[0].p4.pt(), prefix + "evt_lvl_nvtxrewgt_histo_pt_loose_cr_mu", leptons[0].weight * nvtxRewgtMu(leptons[0]), 50, 0, 100);
//                hists.fill(leptons[0].ptratio, prefix + "evt_lvl_nvtxrewgt_histo_ptratio_loose_cr_mu", leptons[0].weight * nvtxRewgtMu(leptons[0]), 50, 0, 1.5);
//                hists.fill(leptons[0].reliso, prefix + "evt_lvl_nvtxrewgt_histo_reliso_loose_cr_mu", leptons[0].weight * nvtxRewgtMu(leptons[0]), 50, 0, 1.5);
//                hists.fill(leptons[0].evt_mt, prefix + "evt_lvl_nvtxrewgt_histo_mt_loose_cr_mu", leptons[0].weight * nvtxRewgtMu(leptons[0]), 50, 0, 200);
//                hists.fill(leptons[0].evt_met, prefix + "evt_lvl_nvtxrewgt_histo_met_loose_cr_mu", leptons[0].weight * nvtxRewgtMu(leptons[0]), 50, 0, 200);
            }
        }
        // EWK CR no MT cut
        if (evt_met            > 30.          &&
            leptons[0].passFO                 &&
            fabs(leptons[0].ip3d) < 0.005     &&
            leptons[0].evt_mt > 80. &&
            leptons[0].p4.pt() > 10.
           )
        {
            if (abs(leptons[0].id) == 13 && fabs(leptons[0].p4.eta()) < 2.4)
            {
                ...
                hists.fill(leptons[0].evt_mt, prefix + "evt_lvl_nvtxrewgt_histo_mt_loose_cr_mu", leptons[0].weight * nvtxRewgtMu(leptons[0]), 50, 0, 200);
                hists.fill(leptons[0].evt_met, prefix + "evt_lvl_nvtxrewgt_histo_met_loose_cr_mu", leptons[0].weight * nvtxRewgtMu(leptons[0]), 50, 0, 200);
                hists.fill(leptons[0].nvtx, prefix + "evt_lvl_nvtxrewgt_histo_nvtx_loose_cr_mu", leptons[0].weight * nvtxRewgtMu(leptons[0]), 50, 0, 50);
            }
        }
        // EWK CR no MT cut
        if (evt_met            > 30.          &&
...
//            std::cout << leptons[0].id << " " << leptons[0].passId << " " << leptons[0].passFO << " " << leptons[0].p4.pt() << " " << evt_met << std::endl;
            ...
            if (abs(leptons[0].id) == 13 && fabs(leptons[0].p4.eta()) < 2.4)
            {
                ...
                {
                    float ptcor = (1 + leptons[0].coneptcorr) * leptons[0].p4.pt();
                    if (ptcor > ptbins[nptbins]) ptcor = ptbins[nptbins] - 1;
                    if (ptbins[ibin] < ptcor && ptbins[ibin+1] > ptcor)
                    {
                        hists.fill(evt_mt, prefix + "evt_lvl_nvtxrewgt_histo_mt_cr_mu" + TString::Format("_%d", ibin).Data(), leptons[0].weight * nvtxRewgtMu(leptons[0]), 20, 0, 200);
                        hists.fill(leptons[0].nvtx, prefix + "evt_lvl_nvtxrewgt_histo_nvtx_cr_mu" + TString::Format("_%d", ibin).Data(), leptons[0].weight * nvtxRewgtMu(leptons[0]), 50, 0, 50);
                        hists.fill(evt_met, prefix + "evt_lvl_nvtxrewgt_histo_met_cr_mu" + TString::Format("_%d", ibin).Data(), leptons[0].weight * nvtxRewgtMu(leptons[0]), 20, 0, 200);
                    }
                }
            }
            if (abs(leptons[0].id) == 11 && fabs(leptons[0].p4.eta()) < 2.5)
            ...
            if (abs(leptons[0].id) == 13 && fabs(leptons[0].p4.eta()) < 2.4)
            { hists.fill(leptons[0].p4.pt(), prefix + "evt_lvl_nvtxrewgt_histo_pt_cr_mu", leptons[0].weight * nvtxRewgtMu(leptons[0]), 50, 0, 100); }
            if (abs(leptons[0].id) == 11 && fabs(leptons[0].p4.eta()) < 2.5)
            { hists.fill(leptons[0].p4.pt(), prefix + "evt_lvl_nvtxrewgt_histo_pt_cr_el", leptons[0].weight * nvtxRewgtEl(leptons[0]), 50, 0, 100); }
        }
        // EWK CR no MT cut
        if (evt_met            < 20.          &&
            leptons[0].passId                 &&
        ...
        //if (abs(leptons[0].id) == 13 && fabs(leptons[0].p4.eta()) < 2.4 && evt_met < 40. && evt_mt < 40. && ((1 + leptons[0].coneptcorr) * leptons[0].p4.pt()) > 40.)
        //if (abs(leptons[0].id) == 13 && fabs(leptons[0].p4.eta()) < 2.4 && evt_met > 20. && evt_met < 40. && ((1 + leptons[0].coneptcorr) * leptons[0].p4.pt()) > 40.)
        if (abs(leptons[0].id) == 13 && fabs(leptons[0].p4.eta()) < 2.4 && evt_met < 20. && evt_mt > 80. && ((1 + leptons[0].coneptcorr) * leptons[0].p4.pt()) > 40.)
        {
            float phi0 = leptons[0].evt_metPhi;
            float phi1 = leptons[0].p4.phi();
            float deltaphi = fabs(TVector2::Phi_mpi_pi(phi0 - phi1));

            if (leptons[0].passId) { hists.fill(deltaphi, prefix + "evt_lvl_histo_deltaphi_meas2_mu", leptons[0].weight, 20, 0, 3.1416); }
            if (leptons[0].passId) { hists.fill((1 + leptons[0].coneptcorr) * leptons[0].p4.pt(), prefix + "evt_lvl_histo_conecorrpt_meas2_mu", leptons[0].weight, 40, 0, 100); }
            if (leptons[0].passId) { hists.fill(evt_met, prefix + "evt_lvl_histo_met_meas2_mu", leptons[0].weight, 40, 0, 40); }
            if (leptons[0].passId) { hists.fill(evt_mt, prefix + "evt_lvl_histo_mt_meas2_mu", leptons[0].weight, 40, 0, 200); }
            if (leptons[0].passId) { hists.fill(leptons[0].nvtx, prefix + "evt_lvl_histo_nvtx_mu", leptons[0].weight, 50, 0, 50); }

            if (leptons[0].passId) { hists.fill(deltaphi, prefix + "evt_lvl_nvtxrewgt_histo_deltaphi_meas2_mu", leptons[0].weight * nvtxRewgtMu(leptons[0]), 20, 0, 3.1416); }
            if (leptons[0].passId) { hists.fill((1 + leptons[0].coneptcorr) * leptons[0].p4.pt(), prefix + "evt_lvl_nvtxrewgt_histo_conecorrpt_meas2_mu", leptons[0].weight * nvtxRewgtMu(leptons[0]), 40, 0, 100); }
            if (leptons[0].passId) { hists.fill(evt_met, prefix + "evt_lvl_nvtxrewgt_histo_met_meas2_mu", leptons[0].weight * nvtxRewgtMu(leptons[0]), 40, 0, 40); }
            if (leptons[0].passId) { hists.fill(evt_mt, prefix + "evt_lvl_nvtxrewgt_histo_mt_meas2_mu", leptons[0].weight * nvtxRewgtMu(leptons[0]), 40, 0, 200); }
            if (leptons[0].passId) { hists.fill(leptons[0].nvtx, prefix + "evt_lvl_nvtxrewgt_histo_nvtx_mu", leptons[0].weight * nvtxRewgtMu(leptons[0]), 50, 0, 50); }
        }
        // MR
        if (evt_met            < 20.          &&
                evt_mt             < 20.          &&
           ...
           )
        {
            if (abs(leptons[0].id) == 13 && fabs(leptons[0].p4.eta()) < 2.4)
            {
                ...
                if ((1 + leptons[0].coneptcorr) * leptons[0].p4.pt() > 40.)
                {
                    float phi0 = leptons[0].evt_metPhi;
                    float phi1 = leptons[0].p4.phi();
                    float deltaphi = fabs(TVector2::Phi_mpi_pi(phi0 - phi1));
                    //if (leptons[0].passFO) { hists.fill((1 + leptons[0].coneptcorr) * leptons[0].p4.pt(), prefix + "evt_lvl_histo_conecorrptvarbin_loose_meas_mu", leptons[0].weight, nptbins, ptbins); }
                    if (leptons[0].passId) { hists.fill(deltaphi, prefix + "evt_lvl_histo_deltaphi_meas_mu", leptons[0].weight, 20, 0, 3.1416); }
                    if (leptons[0].passId) { hists.fill(evt_met, prefix + "evt_lvl_histo_met_meas_mu", leptons[0].weight, 20, 0, 20); }
                }
            ...
            if (abs(leptons[0].id) == 13 && fabs(leptons[0].p4.eta()) < 2.4)
            {
                ...
                if (leptons[0].passId) { hists.fill((1 + leptons[0].coneptcorr) * leptons[0].p4.pt(), prefix + "evt_lvl_nvtxrewgt_histo_conecorrpt_meas_mu", leptons[0].weight * nvtxRewgtMu(leptons[0]), 40, 0, 100); }
                ...
                if (leptons[0].passFO) { hists.fill((1 + leptons[0].coneptcorr) * leptons[0].p4.pt(), prefix + "evt_lvl_nvtxrewgt_histo_conecorrptvarbin_fine_loose_meas_mu", leptons[0].weight * nvtxRewgtMu(leptons[0]), nptbins_fine, ptbins_fine); }
                if (leptons[0].passId) { hists.fill((1 + leptons[0].coneptcorr) * leptons[0].p4.pt(), prefix + "evt_lvl_nvtxrewgt_histo_conecorrptvarbin_fine_meas_mu", leptons[0].weight * nvtxRewgtMu(leptons[0]), nptbins_fine, ptbins_fine); }
                if ((1 + leptons[0].coneptcorr) * leptons[0].p4.pt() > 40.)
                {
                    float phi0 = leptons[0].evt_metPhi;
                    float phi1 = leptons[0].p4.phi();
                    float deltaphi = fabs(TVector2::Phi_mpi_pi(phi0 - phi1));
                    //if (leptons[0].passFO) { hists.fill((1 + leptons[0].coneptcorr) * leptons[0].p4.pt(), prefix + "evt_lvl_histo_conecorrptvarbin_loose_meas_mu", leptons[0].weight, nptbins, ptbins); }
                    if (leptons[0].passId) { hists.fill(deltaphi, prefix + "evt_lvl_nvtxrewgt_histo_deltaphi_meas_mu", leptons[0].weight * nvtxRewgtMu(leptons[0]), 20, 0, 3.1416); }
                }
...
double nvtxRewgtMu(Lepton& lepton)
{
    ...
    //fSumw[2]  = 12.7278    , x = 1.5  , error = 4.75659
    //fSumw[3]  = 8.26908    , x = 2.5  , error = 1.57881
    //fSumw[4]  = 9.65956    , x = 3.5  , error = 1.03299
    //fSumw[5]  = 5.20147    , x = 4.5  , error = 0.346356
    //fSumw[6]  = 3.78385    , x = 5.5  , error = 0.169138
    //fSumw[7]  = 6.98202    , x = 6.5  , error = 0.232178
    //fSumw[8]  = 8.29034    , x = 7.5  , error = 0.213051
    //fSumw[9]  = 4.87242    , x = 8.5  , error = 0.105636
    //fSumw[10] = 3.49568   , x  = 9.5  , error = 0.0652663
    //fSumw[11] = 2.87895   , x  = 10.5 , error = 0.04799
    //fSumw[12] = 2.13881   , x  = 11.5 , error = 0.033073
    //fSumw[13] = 1.78632   , x  = 12.5 , error = 0.0263354
    //fSumw[14] = 1.48913   , x  = 13.5 , error = 0.0215497
    //fSumw[15] = 1.23785   , x  = 14.5 , error = 0.0179469
    //fSumw[16] = 1.04732   , x  = 15.5 , error = 0.0156758
    //fSumw[17] = 0.927559  , x  = 16.5 , error = 0.0144212
    //fSumw[18] = 0.795204  , x  = 17.5 , error = 0.0130903
    //fSumw[19] = 0.729623  , x  = 18.5 , error = 0.0129069
    //fSumw[20] = 0.643646  , x  = 19.5 , error = 0.0123926
    //fSumw[21] = 0.564771  , x  = 20.5 , error = 0.0119759
    //fSumw[22] = 0.518567  , x  = 21.5 , error = 0.0122462
    //fSumw[23] = 0.424935  , x  = 22.5 , error = 0.0114788
    //fSumw[24] = 0.390784  , x  = 23.5 , error = 0.0116108
    //fSumw[25] = 0.314612  , x  = 24.5 , error = 0.0110012
    //fSumw[26] = 0.284379  , x  = 25.5 , error = 0.0112141
    //fSumw[27] = 0.259504  , x  = 26.5 , error = 0.0113464
    //fSumw[28] = 0.21558   , x  = 27.5 , error = 0.0112319
    //fSumw[29] = 0.17168   , x  = 28.5 , error = 0.0105911
    //fSumw[30] = 0.160114  , x  = 29.5 , error = 0.0111147
    //fSumw[31] = 0.110963  , x  = 30.5 , error = 0.0101016
    //fSumw[32] = 0.127262  , x  = 31.5 , error = 0.0119523
    //fSumw[33] = 0.094802  , x  = 32.5 , error = 0.0112928
    //fSumw[34] = 0.0938053 , x  = 33.5 , error = 0.0126297
    //fSumw[35] = 0.108586  , x  = 34.5 , error = 0.0157825
    //fSumw[36] = 0.0904509 , x  = 35.5 , error = 0.0164615
    //fSumw[37] = 0.0667601 , x  = 36.5 , error = 0.016331
    //fSumw[38] = 0.0630477 , x  = 37.5 , error = 0.0191404
    //fSumw[39] = 0.0847914 , x  = 38.5 , error = 0.0284717
    //fSumw[40] = 0.109311  , x  = 39.5 , error = 0.0389476
    //fSumw[41] = 0.305506  , x  = 40.5 , error = 0.0861129
    //fSumw[42] = 0.109834  , x  = 41.5 , error = 0.0636906
    //fSumw[43] = 0.116115  , x  = 42.5 , error = 0.0823888
    //fSumw[44] = 0.109344  , x  = 43.5 , error = 0.109585
    //fSumw[45] = 0.410629  , x  = 44.5 , error = 0.292022
    //fSumw[46] = 0.854452  , x  = 45.5 , error = 0.608776
    //fSumw[47] = 0.75833   , x  = 46.5 , error = 0.761615
    //fSumw[48] = 3.02088   , x  = 47.5 , error = 2.15937
    //fSumw[49] = 2.91525   , x  = 48.5 , error = 2.93519
    //fSumw[50] = 0         , x  = 49.5 , error = 0

    if (nvtx == 1)  { rewgt = 12.7278   ; }
    if (nvtx == 2)  { rewgt = 8.26908   ; }
    if (nvtx == 3)  { rewgt = 9.65956   ; }
    if (nvtx == 4)  { rewgt = 5.20147   ; }
    if (nvtx == 5)  { rewgt = 3.78385   ; }
    if (nvtx == 6)  { rewgt = 6.98202   ; }
    if (nvtx == 7)  { rewgt = 8.29034   ; }
    if (nvtx == 8)  { rewgt = 4.87242   ; }
    if (nvtx == 9)  { rewgt = 3.49568   ; }
    if (nvtx == 10) { rewgt = 2.87895   ; }
    if (nvtx == 11) { rewgt = 2.13881   ; }
    if (nvtx == 12) { rewgt = 1.78632   ; }
    if (nvtx == 13) { rewgt = 1.48913   ; }
    if (nvtx == 14) { rewgt = 1.23785   ; }
    if (nvtx == 15) { rewgt = 1.04732   ; }
    if (nvtx == 16) { rewgt = 0.927559  ; }
    if (nvtx == 17) { rewgt = 0.795204  ; }
    if (nvtx == 18) { rewgt = 0.729623  ; }
    if (nvtx == 19) { rewgt = 0.643646  ; }
    if (nvtx == 20) { rewgt = 0.564771  ; }
    if (nvtx == 21) { rewgt = 0.518567  ; }
    if (nvtx == 22) { rewgt = 0.424935  ; }
    if (nvtx == 23) { rewgt = 0.390784  ; }
    if (nvtx == 24) { rewgt = 0.314612  ; }
    if (nvtx == 25) { rewgt = 0.284379  ; }
    if (nvtx == 26) { rewgt = 0.259504  ; }
    if (nvtx == 27) { rewgt = 0.21558   ; }
    if (nvtx == 28) { rewgt = 0.17168   ; }
    if (nvtx == 29) { rewgt = 0.160114  ; }
    if (nvtx == 30) { rewgt = 0.110963  ; }
    if (nvtx == 31) { rewgt = 0.127262  ; }
    if (nvtx == 32) { rewgt = 0.094802  ; }
    if (nvtx == 33) { rewgt = 0.0938053 ; }
    if (nvtx == 34) { rewgt = 0.108586  ; }
    if (nvtx == 35) { rewgt = 0.0904509 ; }
    if (nvtx == 36) { rewgt = 0.0667601 ; }
    if (nvtx == 37) { rewgt = 0.0630477 ; }
    if (nvtx == 38) { rewgt = 0.0847914 ; }
    if (nvtx >= 39) { rewgt = 0.109311  ; }
    return rewgt / 1.0649;
